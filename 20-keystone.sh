...
PKGS='openstack-keystone httpd mod_wsgi'
...
sed -i '/^$/d' /etc/keystone/keystone.conf
sed -i "/\[database\]/a connection = mysql+pymysql://keystone:$KEYSTONE_DBPASS@${HOST_name[0]}/keystone" /etc/keystone/keystone.conf
sed -i '/\[token\]/a provider = fernet' /etc/keystone/keystone.conf
su -s /bin/sh -c "keystone-manage db_sync" keystone

keystone-manage fernet_setup --keystone-user keystone --keystone-group keystone
keystone-manage credential_setup --keystone-user keystone --keystone-group keystone
keystone-manage bootstrap --bootstrap-password $ADMIN_PASS --bootstrap-admin-url http://${HOST_name[0]}:35357/v3/ --bootstrap-internal-url http://${HOST_name[0]}:5000/v3/ --bootstrap-public-url http://${HOST_name[0]}:5000/v3/ --bootstrap-region-id RegionOne
sed -i "s/^#ServerName www.example.com:80/ServerName ${HOST_name[0]}/g" /etc/httpd/conf/httpd.conf
...
export OS_AUTH_URL=http://${HOST_name[0]}:35357/v3
